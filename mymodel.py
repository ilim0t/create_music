...
class Generator(chainer.Chain):
    def __init__(self, n_voc, n_hidden):
        ...
        with self.init_scope():
            self.l = L.Linear(450*32)

            self.dc1 = L.DeconvolutionND(1, 32, 32, ksize=7, stride=1, pad=1)
            self.dc2 = L.DeconvolutionND(1, 32, 32, ksize=5, stride=1, pad=1)
            self.dc3 = L.DeconvolutionND(1, 32, 32, ksize=3, stride=1, pad=1)
            self.dc4 = L.DeconvolutionND(1, 32, 32, ksize=3, stride=1, pad=1)
            self.dc5 = L.DeconvolutionND(1, 32, 32, ksize=3, stride=1, pad=1)
            self.decoder = L.ConvolutionND(1, 32, n_voc, ksize=1, stride=1)
    ...
    def make_noise(self, batchsize):
        return np.random.uniform(-1, 1, (batchsize, 1, self.n_hidden)).astype(np.float32)

    def __call__(self, z):
        h = self.l(z)
        h = F.reshape(h, [len(h), 32, 450])
        h = F.relu(self.bn0(h))
...
class Discriminator(chainer.Chain):
    def __init__(self, n_vocab, n_vec, n_layers=2, bottom_width=3, ch=512, wscale=0.02):
        ...
        with self.init_scope():
            ...
            self.block2_1 = Bottle_neck_block(32, 64, 3, init_stride=2, pad=1)
            self.block2_2 = Bottle_neck_block(64, 64, 3, pad=1)

            self.block3_1 = Bottle_neck_block(64, 128, 3, init_stride=2, pad=1)
    ...
    def __call__(self, x):
        if isinstance(x, chainer.Variable):
            # h1 = self.embed(F.argmax(x, axis=1))
