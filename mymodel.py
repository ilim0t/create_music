...
class Generator(chainer.Chain):
    def __init__(self, n_voc, n_hidden):
        ...
        with self.init_scope():
            ...
            self.block4_1 = Res_block2(128, 256, 4, init_stride=2)
            self.block4_2 = Res_block2(256, 256, 3)

            # self.decoder = L.ConvolutionND(1, 128, n_voc, ksize=1, stride=1)
            self.decoder2 = L.ConvolutionND(1, 256, 64, ksize=1, stride=1)
...
class Discriminator(chainer.Chain):
    def __init__(self, n_vocab, n_vec, n_layers=2, bottom_width=3, ch=512, wscale=0.02):
        ...
        with self.init_scope():
            ...
            self.block3_1 = Res_block(128, 64, 3, activation=F.leaky_relu, init_stride=2, pad=1)
            self.block3_2 = Res_block(64, 64, 3, activation=F.leaky_relu, pad=1)

            self.block4_1 = Res_block(64, 32, 3, activation=F.leaky_relu, init_stride=2, pad=1)
            self.block4_2 = Res_block(32, 32, 3, activation=F.leaky_relu, pad=1)
